...
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, cognitive_complexity, lines}`. `cognitive_complexity` is the SonarSource nesting-weighted metric (Go + every tree-sitter language except Swift → #491; omitted where unavailable). Go + tree-sitter languages. Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
