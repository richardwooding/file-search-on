...
| `coupling` | Per-package afferent/efferent coupling + instability (Martin metrics) for first-party Go packages: `{package, afferent (Ca), efferent (Ce), instability (I = Ce/(Ca+Ce))}`, ranked most-depended-upon then most unstable — high Ca + high I marks a fragile hub. **Go-only**; run at the module root (resolves first-party packages via the `go.mod` prefix). |
