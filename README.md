...
| `unused_exports` | Exported symbols referenced only from within their own package — candidates to unexport and shrink the public surface. The subtler complement to `dead_code` (used *somewhere*, never across a package boundary). Output: `{symbol, kind, path, package}`. **Go** (capitalised + go.mod path), **Python** (public/`_private` + dir), **Rust** (`pub` + module dir), **TS/JS** (`export` + file), **Java/C#** (`public` + dir); more languages rolling out. **Heuristic** (reflection / external consumers / interface satisfaction mislead). |
