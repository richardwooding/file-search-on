...
| `complexity [expr]` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Also reports SonarSource **cognitive complexity** (nesting-weighted; Go + every tree-sitter language except Swift → #491) | [examples/source-code.md](./examples/source-code.md) |
...
| `coupling [--top N]` | Afferent/efferent coupling + instability (Martin metrics) — fragile architectural seams. Go (packages) + Rust (crates) + JVM Java/Kotlin/Scala (packages) + C# (namespaces) + Python (packages) + JS/TS (directory modules) + PHP (namespaces); more langs → #467 | [examples/source-code.md](./examples/source-code.md) |
...
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, cognitive_complexity, lines}`. `cognitive_complexity` is the SonarSource nesting-weighted metric (Go + every tree-sitter language except Swift; omitted where unavailable). Swift is consistently null — blocked on the upstream tree-sitter-swift `else if` parse bug (#491 / [gotreesitter#131](https://github.com/odvcencio/gotreesitter/issues/131)). Go + tree-sitter languages. Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
