...
| `unused_exports` | Exported Go symbols referenced only from within their own package — candidates to unexport and shrink the public surface. The subtler complement to `dead_code` (used *somewhere*, never across a package boundary). Output: `{symbol, kind, path, package}`. **Go-only**, **heuristic** (reflection / external consumers / interface satisfaction mislead); run at the module root. |
