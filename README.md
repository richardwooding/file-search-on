...
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, lines}`. Go + tree-sitter languages. Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
