...
- **Project-type detection** — `detect-project` / `find-projects` / `which-project` subcommands identify Go / Node / Rust / Python / Ruby / Java / .NET / PHP / Swift / Scala (sbt + Mill) / CMake / Autotools / R / Zig / Perl / MATLAB / Terraform / Docker Compose / Hugo / Jekyll / Eleventy / Astro / Gatsby / MkDocs / Docusaurus / Pelican projects (28 built-in types: 8 SSG + 20 others). Pair with `--resolve-projects` (file-level `project_type` filter) and `--prune-build-artefacts` (skip `vendor`/`node_modules`/`target`/`__pycache__`/`public`/`_site` etc. automatically). The `is_static_site` CEL predicate addresses any SSG as a group. Define custom project types via CEL in YAML — see [examples/projects.md](./examples/projects.md).
...
| `trace <symbol>` | Both directions at once — a symbol's callers + callees (and optional transitive caller closure via `--impact-depth`) in one view | [examples/source-code.md](./examples/source-code.md) |
...
| `complexity [expr]` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Also reports SonarSource **cognitive complexity** (nesting-weighted; Go + every tree-sitter language, Swift included since #491) | [examples/source-code.md](./examples/source-code.md) |
...
| `coupling [--top N]` | Afferent/efferent coupling + instability (Martin metrics) — fragile architectural seams. Go (packages) + Rust (crates) + Swift (SwiftPM target modules) + JVM Java/Kotlin/Scala (packages) + C# (namespaces) + Python (packages) + JS/TS (directory modules) + PHP (namespaces); more langs → #467 | [examples/source-code.md](./examples/source-code.md) |
| `circular` | Circular dependencies — strongly-connected components in the first-party import graph (Tarjan SCC). Same manifest-based language selection as `coupling`, so multi-language | [examples/source-code.md](./examples/source-code.md) |
| `review [--base <ref>]` | Diff-scoped review gate — runs the per-file analyses (cyclomatic + cognitive complexity, dead-code) over the files changed in the git diff and emits a pass/warn/fail verdict; exit code gates CI / pre-commit | [examples/source-code.md](./examples/source-code.md) |
...
The code-analysis commands **`complexity`**, **`dead-code`**, **`find-matches`**, **`coverage-gaps`**, **`unused-exports`**, **`duplicate-functions`**, and **`review`** also accept **`-o sarif`** — a [SARIF 2.1.0](https://sarifweb.azurewebsites.net/) document that [GitHub Code Scanning](https://docs.github.com/en/code-security/code-scanning) (and other CI) ingest, so findings show up as inline PR/code alerts:
...
| **Images** | `img_width`, `img_height`, `camera_make`, `camera_model`, `lens`, `taken_at`, `orientation`, `gps_lat`, `gps_lon`, `iso`, `focal_length`, `f_stop`, `exposure_time`. RAW photos additionally stamp `raw_kind` (`cr2` / `cr3` / `nef` / `arw` / `dng` / `raf` / `orf` / `rw2`) and `raw_vendor` (`canon` / `nikon` / `sony` / `adobe` / `fujifilm` / `olympus` / `panasonic`) — the camera EXIF fields populate via the same `imagemeta` path as JPEG / TIFF. HEIC files paired with a sibling MOV (Apple Live Photos) surface `live_photo_video_path` + `live_photo_video_size`; the MOV side surfaces `live_photo_image_path` and `is_live_photo_video`. With `--with-phash` (CLI) or `with_phash: true` (MCP) — auto-enabled when `image_similar_to(...)` appears in the expression — every image gets a 16-char hex `phash` attribute for perceptual-similarity queries. **C2PA / Content Credentials** (JPEG + PNG): `is_c2pa` flags an embedded provenance manifest and `c2pa_claim_generator` / `c2pa_title` / `c2pa_format` / `c2pa_ai_generated` surface what it claims — the creating tool and whether it declares AI-generated content (`is_image && c2pa_ai_generated`). `c2pa_signed_by` (the COSE signer's x509 certificate common name) and `c2pa_signed_at` (the RFC 3161 signing time) surface *who* claims to have signed it and *when* (`is_c2pa && c2pa_signed_by.contains("Adobe")`). `c2pa_attribution` says who the manifest is a claim **about** — `asset` when the file's own structure associates it, `embedded` when the file associates it with something it *carries* (a PDF object-level manifest over an image or font inside the document), `unknown` when nothing places it. The signer attributes are populated **only** when it is `asset`, so `c2pa_signed_by` never names whoever signed a picture inside a document rather than the document itself; find those files with `is_c2pa && c2pa_attribution != "asset"`. All of the above are read-only and **unverified** — the JUMBF manifest is parsed but its cryptographic signature / trust chain is not validated, so treat it like EXIF. For **verified** provenance, opt in with `--verify-c2pa` (CLI) / `verify_c2pa: true` (MCP): pure-Go validation (COSE signature, certificate chain against the embedded C2PA trust list, hash bindings, RFC 3161 timestamp) then populates `c2pa_valid` (passed full validation), `c2pa_verified_signer` (trust-anchored signer CN/Org), `c2pa_verified_signed_at` (verified timestamp), `c2pa_validation_status` (the first C2PA failure code when invalid), and `c2pa_bound` (what the hard binding proved about *these bytes*: `verified`, `failed`, `unevaluated` or `none`). Find authentic Content Credentials with `is_c2pa && c2pa_valid`, or by verified signer with `c2pa_valid && c2pa_verified_signer.contains("Adobe")`. `c2pa_valid` and `c2pa_bound` answer different questions and come apart in both directions — a manifest describing an image *inside* a PDF is valid with `c2pa_bound == "unevaluated"` because nothing hashed the document, and a file signed with an untrusted certificate is invalid with `c2pa_bound == "verified"` because the bytes really are the signed ones — so provenance that actually covers the file is `is_c2pa && c2pa_valid && c2pa_bound == "verified"`. Off by default — real cryptography per image, and the result is never cached. Verification also surfaces **who vouched for the file** — CAWG identity assertions, where a named actor signed over the content with their own credential, which is a different thing from the tool that wrote the manifest: `c2pa_identity_count`, `c2pa_identity_valid` (any assertion validated), `c2pa_identity_trusted` (any actor PROVEN — effectively always false today, since no identity trust list exists to configure), `c2pa_identity_roles` (`'cawg.creator' in c2pa_identity_roles`) and `c2pa_identity_sig_types` (`cawg.x509.cose` or `cawg.identity_claims_aggregation`). Names are deliberately **not** indexed: an actor's name is unproven until an anchor vouches for it, and CAWG says an identity assertion conveys neither attribution nor ownership, so making unproven personal names searchable would build exactly the claim the spec rules out. |
...
| **Semantic similarity** | `similarity` (double, 0-1) — populated when `--semantic-query` (CLI) / `search_semantic` tool (MCP) is set. Cosine similarity between the file's body embedding and the query embedding, computed via local Ollama. Compose with type predicates: `is_pdf && similarity > 0.7` finds PDFs conceptually related to the query. Vectors cache in the index alongside `(size, mtime)`. Each hit also reports **where** it matched: `match_start_line` / `match_end_line` (the best-matching chunk's line range) and, for **source files** — which are embedded one chunk per function — `match_symbol` (the matching function/method name). Pair with `read_lines` to pull the exact code, or pass `include_match_snippet: true` (MCP `search_semantic`) to inline that region as `match_snippet` on each hit (text/source files only; capped by `snippet_lines`, default 60). |
| **Keyword relevance** | `bm25` (double) — populated when `--keyword-query` (CLI) / `keyword_query` (MCP) is set. Okapi BM25 keyword-relevance score, with IDF computed over the matched candidate set. Compose with `similarity` in a `--rank` expression for hybrid keyword+semantic ranking (`bm25*0.4 + similarity*0.6`), or use `--hybrid` for automatic reciprocal-rank fusion. Only comparable within one result set. |
...
| `search_semantic` | Natural-language similarity search via local Ollama embeddings. Pre-prunes with an optional `expr`, embeds the query, ranks files by cosine similarity, applies a `threshold` cap, and supports `cursor` / `next_cursor` pagination. Embeddings cache per file. After a tree's first walk, repeat queries are served from a **warm in-memory vector index** (no walk, no re-embedding; `ann_used` reports the path, with structural-change fallback + per-file staleness detection). `hybrid: true` fuses BM25 keyword relevance with similarity via reciprocal-rank fusion. |
...
| `find_duplicate_functions` | Clusters of near-identical **functions** across the tree — copy-pasted logic the file-level scan misses. Splits source into per-function spans, SimHashes each body, groups within a threshold (default 0.92). Each member carries `{path, symbol, start_line, end_line, lines}` — `read_lines` the span to see it. `min_lines` (default 5) filters trivial functions. |
...
| `api_diff` | Breaking-change gate: the exported function/type names `removed` (the breaking set) vs `added` between `tree_a` (baseline) and `tree_b` (candidate). `breaking=true` iff anything was removed. "Exported" = upper-cased first rune (Go's rule exactly; a heuristic elsewhere — narrow with `expr`). v1 compares name + kind presence, not signatures. |
...
| `references` | Every **usage** of a `symbol` with `{path, line, kind}` — the IDE "find references" primitive and the complement to `find_definition`. `kind` is `call` / `type` (field/param/return/generic-arg) / `value` (Go callback). Pre-filters to referencing files via the graph, then parses only those for exact lines. Go + tree-sitter (calls + type usages; Go-only for value). Heuristic, name-based. |
...
| `who_calls` | Reverse usage lookup: every file that calls/references a `symbol`. "Who uses Y?" — the third leg of the graph. References extracted for Go + the tree-sitter languages. Also covers **type usages** (field/param/return/generic-arg types, #398) for Go and the statically-typed tree-sitter languages (Rust / TS / Python / Java / C# / C / C++ / Kotlin / Swift / Scala / PHP), so querying a type name finds its users, not just constructor calls — plus, for Go, **function values passed as callbacks** (#421), so a handler registered via `AddTool`/`HandleFunc` shows its registration site. |
| `impact` | Transitive reverse-dependency closure for a function — every function that (in)directly calls `symbol`, with the depth each was reached at. The blast radius before a refactor; extends `who_calls` (one hop) to the full closure. Heuristic (name-based call graph). |
| `call_path` | Shortest call path from one function to another — *how* `from` reaches `to` through the call graph (the route, where `impact` gives the closure). Output: the ordered `path[]` of `{symbol, defining files}`, `reachable`, `length`. Heuristic (name-based call graph). |
| `calls` | Forward call lookup: the distinct functions a given `symbol` calls. "What does Y call?" Per-function attribution via span-containment; same languages as `who_calls`. Name-based (callees keyed by bare name). |
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, cognitive_complexity, lines}`. `cognitive_complexity` is the SonarSource nesting-weighted metric (Go + every tree-sitter language, Swift included since #491 / [gotreesitter#131](https://github.com/odvcencio/gotreesitter/issues/131); omitted only for a language with no cognitive spec). Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
| `dead_code` | Candidate unreferenced definitions (functions/types nothing calls or uses). The graph tracks **type usages** (#398) and **function values passed as callbacks** (#421, the `AddTool`/`HandleFunc` pattern) as well as calls, and excludes Go `init`/`main` + test/`…Cmd` entry points — so those whole false-positive classes are gone. **Heuristic** — exported API used externally, interface methods, dynamic dispatch, and reflection still produce false positives; pair with `expr: 'is_source && !is_test_file'` and treat as a review list, not a delete list. |
| `test_gaps` | Production files whose functions are never referenced from a test — candidate untested code, no coverage instrumentation needed. Output ranks fully-untested files first, each with its `untested_functions[]`. **Heuristic** (direct test references only) — transitively-exercised code can appear; for precise coverage use a profile. |
| `coverage_gaps` | Functions below a coverage threshold, from a Go coverage profile (`go test -coverprofile`). The precise complement to `test_gaps` — catches partially-tested functions and counts transitive coverage — but needs the profile. Output: `{function, covered_pct, covered/total statements, line range}`, worst-coverage first. |
| `coupling` | Afferent/efferent coupling + instability (Martin metrics) over first-party nodes: `{package, afferent (Ca), efferent (Ce), instability (I = Ce/(Ca+Ce))}`, ranked most-depended-upon then most unstable — high Ca + high I marks a fragile hub. Granularity is picked by the manifest at the root: **Go** (`go.mod`) → packages, **Rust** (`Cargo.toml`) → crates, **JVM** — Java / Kotlin / Scala (Maven / Gradle / `build.sbt`) → packages, **C#** (`.sln` / `.csproj`) → namespaces, **Python** (`pyproject.toml` / `setup.py`) → packages, **JS/TS** (`package.json` / `tsconfig.json`) → directory modules, **PHP** (`composer.json`) → namespaces, **Swift** (`Package.swift`) → SwiftPM target modules. More languages tracked in #467. |
| `unused_exports` | Exported symbols referenced only from within their own package — candidates to unexport and shrink the public surface. The subtler complement to `dead_code` (used *somewhere*, never across a package boundary). Output: `{symbol, kind, path, package}`. 9 languages — **Go** (capitalised + go.mod path), **Python** (public/`_private`), **Rust** (`pub`), **TS/JS** (`export`, file=module), **Java/C#** (`public` + dir), **Kotlin/Scala** (default-public minus private/internal). **Heuristic** (reflection / external consumers / interface satisfaction mislead). |
| `churn_owners` | Ownership / bus-factor per directory — which subtrees are effectively single-maintainer. Aggregates git authorship over tracked files: `{dir, files, distinct_authors, top_author, top_author_share, total_commits}`, ranked fewest-authors-then-highest-churn. **Approximate** — keys on `git_last_commit_author` (last committer), not a full blame. Requires a git working tree. |
...
Every walking tool (`search`, `stats`, `find_duplicates`, `find_near_duplicates`, `find_duplicate_functions`, `find_matches`, `imported_by`, `find_definition`, `code_graph`, `who_calls`, `calls`, `impact`, `call_path`, `dead_code`, `test_gaps`, `complexity`, `find_projects`, `diff_trees`) honours the same partial-result contract: on timeout the call returns `cancelled=true` with the results gathered so far, never an error. Agents inspect the flag rather than catching exceptions.
