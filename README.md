...
| `who_calls` | Reverse usage lookup: every file that calls/references a `symbol`. "Who uses Y?" — the third leg of the graph. References extracted for Go + the tree-sitter languages (Rust / TS / JS / Ruby / Swift / Kotlin / C / C++). For Go this also covers **type usages** (field/param/embedding/generic-arg types, #398), so querying a type name finds its users, not just constructor calls. |
...
| `dead_code` | Candidate unreferenced definitions (functions/types nothing calls or uses). For Go the graph tracks **type usages** as well as calls (#398), so a type used only as a field/param type is no longer a false positive. **Heuristic** — exported API, entry points, dynamic dispatch, and reflection still produce false positives; pair with `expr: 'is_source && !is_test_file'` and treat as a review list, not a delete list. |
