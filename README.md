...
| `complexity [expr]` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Also reports SonarSource **cognitive complexity** (nesting-weighted; Go + Python / JS / TS / Java / Rust / C / C++ / C# / Kotlin / PHP, remaining languages → #491) | [examples/source-code.md](./examples/source-code.md) |
...
| `review [--base <ref>]` | Diff-scoped review gate — runs the per-file analyses (complexity, dead-code) over the files changed in the git diff and emits a pass/warn/fail verdict; exit code gates CI / pre-commit | [examples/source-code.md](./examples/source-code.md) |
...
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, cognitive_complexity, lines}`. `cognitive_complexity` is the SonarSource nesting-weighted metric (Go + Python / JS / TS / Java / Rust / C / C++ / C# / Kotlin / PHP; remaining languages → #491, omitted where unavailable). Go + tree-sitter languages. Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
