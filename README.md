...
Every walking tool (`search`, `stats`, `find_duplicates`, `find_near_duplicates`, `find_duplicate_functions`, `find_matches`, `imported_by`, `find_definition`, `code_graph`, `who_calls`, `calls`, `dead_code`, `complexity`, `find_projects`, `diff_trees`) honours the same partial-result contract: on timeout the call returns `cancelled=true` with the results gathered so far, never an error. Agents inspect the flag rather than catching exceptions.
