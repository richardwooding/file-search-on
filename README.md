...
| `who_calls` | Reverse call lookup: every file that calls/references a `symbol`. "Who calls Y?" — the third leg of the graph. References extracted for Go + the tree-sitter languages (Rust / TS / JS / Ruby / Swift / Kotlin / C / C++). |
...
| `dead_code` | Candidate unreferenced definitions (functions/types nothing calls). **Heuristic** — exported API, entry points, dynamic dispatch, and reflection produce false positives; pair with `expr: 'is_source && !is_test_file'` and treat as a review list, not a delete list. |
