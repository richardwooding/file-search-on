...
| `complexity [expr]` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Also reports SonarSource **cognitive complexity** (nesting-weighted; Go today, other languages → #491) | [examples/source-code.md](./examples/source-code.md) |
...
| `complexity` | Functions ranked by cyclomatic complexity (worst-first) — maintenance hotspots. Rows: `{path, function, complexity, cognitive_complexity, lines}`. `cognitive_complexity` is the SonarSource nesting-weighted metric (Go today; other languages → #491, omitted where unavailable). Go + tree-sitter languages. Directional ranking, not a certified metric. Pairs with the `max_complexity` search attribute for file-level filtering. |
