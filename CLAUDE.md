...
- **`internal/projecttype`** — directory-granularity counterpart to `internal/content`. Each `ProjectType` has `Indicator`s (`HasFile` / `HasGlob` / `CELExpr` over `files` + `subdirs`). `Detect(fsys, dir)` returns every matching type; `Find(ctx, root, opts)` walks recursively. `ProjectResolver` answers "what project does this file belong to?" by walking up from the file's parent (cached per-dir). Surfaced as `project_types` / `project_type` CEL vars when `search.Options.ResolveProjects` is set. `BuildExcludes` drives `--prune-build-artefacts`. 18 built-ins in `builtins.go`; custom types loadable via YAML (`config.go` + auto-discovery in `discovery.go`).
...
3. Add a `case` for the new content-type name in `setTypeFlags` (`internal/celexpr/evaluator.go`) — and, if it belongs to a family (`build/*`, `repo/*`, `manifest/*`, `system/*`, etc.), rely on the matching prefix-`if` block at the bottom.
