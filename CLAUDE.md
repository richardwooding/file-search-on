...
- **`internal/mcpserver`** — exposes the same search via the [official MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk). `New(version, idx)` builds an `*mcp.Server` with four tools (`search` thinly wraps `search.WalkStream`; `read_attributes` calls `BuildAttributesWith` for a single path and returns the same `SearchMatch` shape; `list_attributes` returns `celexpr.Schema()` plus the registered content types; `index_stats` returns the cache's hit/miss/put/stale/error counters). Tool handlers are methods on a per-server `handlers` struct that holds the shared `index.Index`. Three transport runners: `Run(ctx, version, idx)` for stdio, `RunHTTP(ctx, version, addr, path, idx)` for the modern Streamable HTTP transport (MCP 2025-03-26), and `RunSSE(...)` for the deprecated HTTP+SSE transport (MCP 2024-11-05, kept for back-compat). The HTTP/SSE runners wrap the SDK's `StreamableHTTPHandler` / `SSEHandler` in a stdlib `http.Server` and shut down on ctx cancellation. The `mcp` CLI subcommand picks the runner via `--transport`.
