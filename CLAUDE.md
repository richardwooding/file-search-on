...
When adding/removing a tree-sitter language, keep three places in sync: the language set in `treesitter-symbols` (and `go-codemetrics/treesitter` for metrics — the extracted modules), `symbolExtractorWired` in `sourcetype.go`, and the `tags:` list in `.goreleaser.yaml`.
