...
- **`internal/mcpserver`** — exposes the same search via the [official MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk). `New(version)` builds an `*mcp.Server` with three tools (`search` thinly wraps `search.Walk`; `read_attributes` calls `BuildAttributes` for a single path and returns the same `SearchMatch` shape; `list_attributes` returns `celexpr.Schema()` plus the registered content types). Three transport runners: `Run(ctx, version)` for stdio (the default for desktop clients), `RunHTTP` for the modern Streamable HTTP transport (MCP 2025-03-26), and `RunSSE` for the deprecated HTTP+SSE transport (MCP 2024-11-05, kept for back-compat). The HTTP/SSE runners wrap the SDK's `StreamableHTTPHandler` / `SSEHandler` in a stdlib `http.Server` and shut down on ctx cancellation. The `mcp` CLI subcommand picks the runner via `--transport`.
