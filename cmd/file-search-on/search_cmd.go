...
type SearchCmd struct {
	...
	OCR              bool          `name:"ocr" help:"Run OCR over image/* files via the registered OCR provider (macOS Vision today; Linux Tesseract / Windows.Media.Ocr are future providers). Populates 'body' with the recognized text plus ocr_confidence (0..1), ocr_language (BCP-47), ocr_provider. Works independently of --body — passing --ocr alone is enough for 'is_image && body.contains(\"kubernetes\")' queries. Cached in the body cache (bodies_v1) so the second walk over the same tree is free. Expensive on first walk (200ms-2s per image). On platforms without a registered provider, the flag is a no-op. Issue #189."`
	OCRTimeout       time.Duration `name:"ocr-timeout" default:"10s" help:"Per-file timeout for the OCR call. Default 10s; the helper subprocess gets SIGKILL on ctx cancellation so a misbehaving image can't stall the walk."`
	VerifyC2PA       bool          `name:"verify-c2pa" help:"Cryptographically VERIFY C2PA / Content Credentials on JPEG, PNG, WebP, GIF, TIFF, DNG, HEIC, SVG, MP4/MOV, AVI, WAV, MP3 and PDF files (pure-Go: COSE signature, certificate chain against the embedded C2PA trust list, hash bindings, RFC 3161 timestamp) and expose c2pa_valid / c2pa_verified_signer / c2pa_verified_signed_at / c2pa_validation_status / c2pa_bound — the authenticated counterpart to the always-on, unverified c2pa_* attributes. Off by default: real crypto per file, never cached. Example: --verify-c2pa 'is_c2pa && c2pa_valid && c2pa_verified_signer.contains(\"Adobe\")'. Issue #441."`
	WithPHash        bool          `name:"with-phash" help:"Compute the 64-bit perceptual hash of every walked image and surface it as the 'phash' CEL string (16-char hex). Pair with the image_similar_to(phash, ref_path, threshold) CEL function to find visually-similar images. Auto-enabled when the expression references image_similar_to. Cached in the index alongside (size, mtime). ~1ms per image. Pure-Go DCT, no third-party deps. Issue #208."`
	...
	IncludeGit       bool          `name:"include-git" help:"Walk into .git directories. By default .git is pruned from every walk (its objects/refs/logs are never searched or indexed); pass this to include it. Pointing -d directly at a .git dir works without the flag (the walk root is exempt)."`
...
func (s *SearchCmd) Run(ctx context.Context) error {
	...
	opts := search.Options{
		...
		OCRImages:         s.OCR,
		OCRTimeout:        s.OCRTimeout,
		VerifyC2PA:        s.VerifyC2PA,
		WithPHash:         s.WithPHash || strings.Contains(s.Expr, "image_similar_to"),
