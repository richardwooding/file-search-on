...
var CLI struct {
	...
	Coupling           CouplingCmd           `cmd:"" name:"coupling" help:"Per-package afferent/efferent coupling + instability (Martin metrics) for first-party Go packages — the fragile-hub seams (high Ca + high I) a refactor is riskiest near. Go-only; run at the module root. e.g. coupling -d . --top 20."`
	UnusedExports      UnusedExportsCmd      `cmd:"" name:"unused-exports" help:"Exported symbols referenced only within their own package — candidates to unexport and shrink the public API surface. The subtler complement to dead-code. Go / Python / Rust / TypeScript / JavaScript / Java / C# (more languages rolling out). e.g. unused-exports -d ."`
