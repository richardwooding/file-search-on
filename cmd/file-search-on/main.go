...
var CLI struct {
	...
	Coupling           CouplingCmd           `cmd:"" name:"coupling" help:"Afferent/efferent coupling + instability (Martin metrics) — the fragile-hub seams (high Ca + high I) a refactor is riskiest near. Granularity is picked by the manifest at the root: Go (go.mod) → packages, Rust (Cargo.toml) → crates, Java (pom.xml / Gradle) → packages, C# (.sln / .csproj) → namespaces. More languages tracked in #467. e.g. coupling -d . --top 20."`
