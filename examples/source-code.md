...
Cognitive complexity is computed for **Go** (precise, matching the SonarSource reference examples) plus **every tree-sitter language** (the same nesting-weighted walk over their parse trees). Swift joined the set once [gotreesitter#131](https://github.com/odvcencio/gotreesitter/issues/131) fixed the tree-sitter-swift `else if` mis-parse (#491); its else-if chain is charged the flat continuation cost, not a nested-if penalty. `-o json` carries `cognitive_complexity` per function (omitted only for a language with no cognitive spec), and `-o sarif` includes it in the finding message. Same data over MCP via the `complexity` tool.
...
# 5   1   0.17         github.com/you/m/internal/content   ← stable core (many depend on it)
...
- **Swift** (`Package.swift`) → **SwiftPM target modules**. A Swift module is a build target, not a directory, so nodes are the package's targets; first-party = the target names declared in `Package.swift` (parsed pragmatically from `.target` / `.executableTarget` / `.testTarget`, plus the conventional `Sources/<Target>/` and `Tests/<Target>/` layout). An `import MyLib` edges into target `MyLib` (and `import MyLib.Sub` resolves to the leading module `MyLib`); `Foundation` and external SwiftPM package modules are ignored. Exotic manifests (computed target lists, custom `sources:` arrays) may resolve imperfectly (#520 / #477).
- **JVM — Java / Kotlin / Scala** (Maven `pom.xml` / Gradle / `build.sbt`) → **packages**. All three share the `package com.foo.bar` model, so one adapter graphs a mixed JVM project. Nodes are declared packages; first-party = the set of packages the tree itself declares (no build-file parsing — works across Maven, Gradle, sbt, and plain source trees). An `import com.x.Y` is an edge into package `com.x`; JDK and third-party imports (`java.util.*`, `kotlin.*`, `scala.*`, …) are ignored because those packages are never declared in-tree.
...
- **Python** (`pyproject.toml` / `setup.py`) → **packages**. A file's package is the dotted path of its directory beneath the *import root* — a top-level `src/` when present (src-layout), else the project root. Both absolute imports (`import a.b.c`, `from a.b import c`) and **relative imports** (`from . import x`, `from ..pkg import y`) are counted: relative imports resolve against the importing file's own package (N dots ascend N−1 levels, then the remainder is appended), so idiomatic packages like flask — which import internally via relative imports — produce a real graph. Stdlib and third-party imports (`os`, `numpy`, …) are ignored.
...
- **PHP** (`composer.json`) → **namespaces**. Same model as the JVM family but backslash-separated: node = the declared `namespace App\Services`, first-party = the namespaces the tree declares. A `use App\Util\Helper;` edges into namespace `App\Util`; vendor / third-party namespaces (`Psr\Log\*`, …) are ignored. Selected before `package.json` so a PHP app with a JS frontend build (Laravel, Symfony) graphs its PHP backend.
...
Each line is one cycle — the mutually-dependent members of a strongly-connected component — ranked largest-first; `[N]` is its size. (Members are listed as a set, not an edge path: a strongly-connected group has no single canonical loop order.) Because it shares `coupling`'s manifest-based language selection, it works for Go, Rust, Swift, JVM (Java/Kotlin/Scala), C#, Python, JS/TS, and PHP — point `-d` at the project root. `-o json` emits `{module, cycles: [{nodes, length}], count}`. Self-imports are never reported.
...
Works across 9 languages — **Go** (capitalised name + go.mod import path), **Python** (public/`_private` + package directory), **Rust** (`pub` + module directory), **TypeScript / JavaScript** (`export` + the file as ES module), **Java / C#** (`public` keyword + directory), and **Kotlin / Scala** (default-public minus private/internal/protected + directory). PHP, Ruby, Swift, Perl, R and MATLAB are out of scope (no usable export/visibility concept for this check). Built on the cross-language type-usage tracking (a type used as a field type in *another* package correctly disqualifies it). **Heuristic** — reflection / framework dispatch (kong `…Cmd`, Go test entry points) is excluded, but symbols kept exported for unit-testability, interface satisfaction, or external consumers outside the tree still show up. A review list, not an auto-unexport list.
