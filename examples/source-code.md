...
Cognitive complexity is computed for **Go** (precise, matching the SonarSource reference examples) plus **every tree-sitter language except Swift** (the same nesting-weighted walk over their parse trees). Swift is deliberately omitted — its `COGNITIVE` column shows `—` everywhere (consistently null, not a mix) because the upstream tree-sitter-swift grammar mis-parses `else if` chains (it emits ERROR nodes and degrades the enclosing function), which would make any cognitive number unreliable for real code; cyclomatic is still reported on a best-effort basis. Tracked in #491 (blocked on upstream [odvcencio/gotreesitter#131](https://github.com/odvcencio/gotreesitter/issues/131)). `-o json` carries `cognitive_complexity` per function (omitted where unavailable), and `-o sarif` includes it in the finding message. Same data over MCP via the `complexity` tool.
