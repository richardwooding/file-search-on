...
Cognitive complexity is computed for **Go** (precise, matching the SonarSource reference examples) plus **every tree-sitter language except Swift** (the same nesting-weighted walk over their parse trees). For Swift the `COGNITIVE` column shows `—` (not yet available — tracked in #491); cyclomatic is reported for it too. `-o json` carries `cognitive_complexity` per function (omitted where unavailable), and `-o sarif` includes it in the finding message. Same data over MCP via the `complexity` tool.
