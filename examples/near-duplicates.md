...
- **Threshold is a precision/recall knob.** Lower threshold = more matches (recall) but more false positives (precision). 0 (default) lets the engine pick: **0.92 for source-dominated trees, 0.85 elsewhere**. Source code's idiomatic tokens (Go's `func` / `err` / `nil` / `ctx`, Python's `def` / `import` / `self`) push EVERY file's raw SimHash into a similar bit pattern regardless of content — even after the per-language boilerplate strip (license headers / `package` / `import` lines) removes the most obvious noise. The tighter default cuts the false-positive rate on code without burying real template-copy / regenerated-boilerplate near-duplicates. Override explicitly when you want the looser 0.85 (or stricter 0.95 typo-edits-only) behaviour. Issue #274.
