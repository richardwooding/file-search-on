...
| [`astronomy.md`](./astronomy.md) | FITS + VOTable + HDF5 + PDS3/PDS4 + CDF — the full `science/*` family. FITS: `telescope`, `instrument`, `object`, `observer`, `exptime`, `ra`, `dec`, `bitpix`, `naxis*`, `hdu_count`, `fits_kind`. VOTable: `votable_version`, `table_count`, `total_rows`, `field_names`, `field_units`, `field_ucds`, `votable_data_format`. HDF5: `hdf5_format_version`, `hdf5_size_of_offsets`, `hdf5_size_of_lengths`. PDS: `pds_version`, `mission_name`, `spacecraft_name`, `instrument_name`, `target_name`, `product_id`, `start_time`. CDF: `cdf_version`, `cdf_encoding`, `cdf_majority`, `variable_count`, `attribute_count` |
...
| [`chat-exports.md`](./chat-exports.md) | Slack / Discord / signal-cli JSON exports — `is_chat_export`, `chat_participants`, `chat_channel`, message-body search via `--body`. Detected by top-level JSON shape |
...
| [`databases.md`](./databases.md) | SQLite v3 — `database_format`, `sqlite_page_size`, `sqlite_format_version` (1 legacy / 2 WAL), `sqlite_page_count`, `sqlite_schema_version`, `sqlite_text_encoding`, `sqlite_user_version`, `sqlite_application_id`. Header-only detection for `.db` / `.sqlite` / `.sqlite3`. |
...
| [`diff.md`](./diff.md) | Cross-tree set operations by sha256 — `file-search-on diff <tree-a> <tree-b> --op a-minus-b`. Pre-backup sanity, sync correctness, drift detection; `diff_trees` MCP tool |
