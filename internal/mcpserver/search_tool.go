...
type SearchInput struct {
	...
	WithGit         bool     `json:"with_git,omitempty" jsonschema:"When true, populate git-aware CEL variables (git_last_commit_time, git_last_commit_author, git_last_commit_subject, git_first_seen, git_commit_count, is_git_tracked, is_git_ignored) by running one 'git log' pass per walk root via the gitmeta package. Cheap when the root IS a git tree (a single subprocess invocation up front); silent no-op when the root isn't (or when git isn't on PATH). Use to answer repo-aware time / author / churn queries that filesystem mtimes can't (a fresh clone has all mtimes set to checkout time). Filter examples: 'git_last_commit_time > timestamp(...)' (recent edits), 'is_source && git_commit_count > 50' (hot files), 'is_source && is_git_tracked && !is_test_file' (production code only). Issue #271."`
