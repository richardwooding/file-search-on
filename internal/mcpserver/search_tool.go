...
type SearchInput struct {
	...
	VerifyC2PA       bool     `json:"verify_c2pa,omitempty" jsonschema:"When true, cryptographically VERIFY C2PA / Content Credentials on JPEG, PNG, WebP, GIF, TIFF, DNG, HEIC, SVG, MP4/MOV, AVI, WAV, MP3 and PDF files (pure-Go: COSE signature, certificate chain against the embedded C2PA trust list, hash bindings, RFC 3161 timestamp) and populate the VERIFIED attributes: c2pa_valid (bool — passed full validation), c2pa_verified_signer (trust-anchored signer cert CN/Org), c2pa_verified_signed_at (verified RFC 3161 timestamp), c2pa_validation_status (first C2PA failure code when invalid), c2pa_bound (string — what the hard binding proved about THESE bytes: 'verified', 'failed', 'unevaluated' when a binding exists but was not checked against the file (an object-level PDF manifest, a fragmented video without its fragments), 'none'). c2pa_valid and c2pa_bound are different questions and come apart both ways, so provenance that covers the file is 'is_c2pa && c2pa_valid && c2pa_bound == \"verified\"'. These are the authenticated counterpart to the always-on, UNVERIFIED is_c2pa / c2pa_signed_by / c2pa_signed_at attributes. Off by default: real crypto per file, and the result is never cached (validity is clock-dependent). Find authentic Content Credentials: 'is_c2pa && c2pa_valid'; by verified signer: 'c2pa_valid && c2pa_verified_signer.contains(\"Adobe\")'. Issue #441."`
	...
	IncludeGit          bool  `json:"include_git,omitempty" jsonschema:"When true, walk into .git directories. By default .git is pruned from every walk (its objects/refs/logs are never searched or indexed); set this to include it. Pointing dir directly at a .git directory works without this flag (the walk root is exempt)."`
