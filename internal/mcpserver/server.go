...
Body-content filters: pass 'include_body': true to expose the full file body to the CEL expression as the 'body' string variable. CEL's built-in string methods then act as content filters — body.contains("transformer"), body.matches("\\bAPI\\b") (RE2 regex), body.startsWith("Once upon"), size(body) > 5000. Text-shaped types (markdown / text / html / csv / json / xml / source/*) populate via a raw byte read; structured-document types (office/docx / office/xlsx / office/pptx / office/odt / epub / email/rfc822 / email/mbox) populate via a format-specific extractor. Office and EPUB extractors walk the ZIP envelope and pull plain text out of the content XML (word/document.xml, sharedStrings.xml + inline-string cells, ppt/slides/slideN.xml, content.xml, OPF spine → (X)HTML chapters). Email extractors walk the MIME tree: multipart/alternative prefers text/plain over text/html; multipart/mixed / related concatenates every non-attachment text part; Content-Transfer-Encoding (quoted-printable / base64) is decoded; mbox archives concatenate every message's body so body.contains("invoice") searches an entire inbox in one call. Headers (Subject / From / etc.) are NOT in the body — they surface as separate CEL variables (title / author / email_to / sent_at). Attachments are skipped. So body.contains("Q3 revenue") works on a spreadsheet, an EPUB chapter, an .eml, or an mbox the same way it works on a markdown file. Capped at body_max_bytes (default 1 MiB, applied to EXTRACTED text — not raw file size — so a 50 MB mbox with sparse text reads cheaply). EXPENSIVE — reads every candidate file. Pair with a tight expr (e.g. 'is_email && body.contains(...)') so the type predicate prunes most candidates before the body read. Note: CEL's 'matches' uses RE2 (Google's regex syntax), the same engine Go's regexp/re2 package uses.
