...
Body-content filters: pass 'include_body': true to expose the full file body to the CEL expression as the 'body' string variable. CEL's built-in string methods then act as content filters — body.contains("transformer"), body.matches("\\bAPI\\b") (RE2 regex), body.startsWith("Once upon"), size(body) > 5000. Text-shaped types (markdown / text / html / csv / json / xml / source/*) populate via a raw byte read; structured-document types (office/docx / office/xlsx / office/pptx / office/odt / epub) populate via a format-specific extractor that walks the ZIP envelope, pulls plain text out of the content XML (word/document.xml, sharedStrings.xml + inline-string cells, ppt/slides/slideN.xml, content.xml, OPF spine → (X)HTML chapters), and joins with paragraph breaks. So body.contains("Q3 revenue") works on a spreadsheet the same way it does on a markdown file. Capped at body_max_bytes (default 1 MiB, applied to EXTRACTED text — not raw file size — so a 50 MB PPTX with sparse text reads cheaply). EXPENSIVE — reads every candidate file. Pair with a tight expr (e.g. 'is_office && body.contains(...)') so the type predicate prunes most candidates before the body read. Note: CEL's 'matches' uses RE2 (Google's regex syntax), the same engine Go's regexp/re2 package uses.
