...
func New(version string, idx index.Index, defaultTimeout time.Duration, embedDefaults EmbedDefaults, opts ...Option) *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{
		Name:        "who_calls",
		Description: "Cross-file reverse call lookup: list every source file that calls/references a given function or method name. The third leg of the code graph (complements imported_by for modules and find_definition for definitions). Input: symbol (required, exact name — e.g. 'ServeHTTP', 'process'), plus expr (defaults to is_source) / dir / dirs / excludes / respect_gitignore / timeout_seconds. Reference extraction covers Go (go/ast) + the tree-sitter languages (Rust / TypeScript / JavaScript / Ruby / Swift / Kotlin / C / C++); callers written in other languages won't appear. Name-based resolution: a call pkg.Foo() or x.Method() is keyed by 'Foo' / 'Method', so same-named symbols across types/packages collapse together (reliable for 'who calls X', heuristic for disambiguation). Output: callers[] {path, language} sorted by path + count + total_files.",
	}, instrument(h.metrics, "who_calls", h.whoCallsHandler))

	mcp.AddTool(s, &mcp.Tool{
		Name:        "dead_code",
		Description: "List candidate unreferenced definitions — functions / types defined somewhere in the tree whose name never appears as a call/reference anywhere in it. Built from the cross-file reference graph. Input: expr (defaults to is_source) / dir / dirs / excludes / respect_gitignore / timeout_seconds. Restricted to definitions in languages with reference extraction (Go + Rust / TypeScript / JavaScript / Ruby / Swift / Kotlin / C / C++) so languages we don't scan for calls aren't reported wholesale. Reflection-dispatched entry points that would always be false positives are excluded: Go test-runner functions (TestXxx / BenchmarkXxx / FuzzXxx / ExampleXxx in *_test.go) and kong/cobra-style *Cmd command types. IMPORTANT — the rest are still CANDIDATES, not authoritative: name-based heuristic with known false positives — exported/public API used only by external callers, entry points (main), dynamic dispatch, reflection, and same-name collisions all surface here. Type-level results are especially weak: the graph tracks call references, not type usages, so a type used only as a field/value type can look unreferenced. Use as a starting point for manual review, never as a delete list. Output: candidates[] {path, language, kind, symbol} sorted by path + count + total_files.",
