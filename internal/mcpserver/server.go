...
func New(version string, idx index.Index, defaultTimeout time.Duration, embedDefaults EmbedDefaults, opts ...Option) *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{
		Name:        "who_calls",
		Description: "Cross-file reverse usage lookup: list every source file that calls/references a given function or type name. The third leg of the code graph (complements imported_by for modules and find_definition for definitions). Input: symbol (required, exact name — e.g. 'ServeHTTP', 'process', 'Widget'), plus expr (defaults to is_source) / dir / dirs / excludes / respect_gitignore / timeout_seconds. Reference extraction covers Go (go/ast) + the tree-sitter languages (Rust / TypeScript / JavaScript / Ruby / Swift / Kotlin / C / C++); callers written in other languages won't appear. For Go it also picks up TYPE usages (a type named as a field/parameter/composite-literal/embedding/generic-arg type; issue #398), so querying a type name finds its users, not just constructor calls; other languages capture call sites only. Name-based resolution: a call pkg.Foo() or x.Method() is keyed by 'Foo' / 'Method', so same-named symbols across types/packages collapse together (reliable for 'who uses X', heuristic for disambiguation). Output: callers[] {path, language} sorted by path + count + total_files.",
	}, instrument(h.metrics, "who_calls", h.whoCallsHandler))

	mcp.AddTool(s, &mcp.Tool{
		Name:        "dead_code",
		Description: "List candidate unreferenced definitions — functions / types defined somewhere in the tree whose name never appears as a call/reference anywhere in it. Built from the cross-file reference graph. Input: expr (defaults to is_source) / dir / dirs / excludes / respect_gitignore / timeout_seconds. Restricted to definitions in languages with reference extraction (Go + Rust / TypeScript / JavaScript / Ruby / Swift / Kotlin / C / C++) so languages we don't scan for calls aren't reported wholesale. Reflection-dispatched entry points that would always be false positives are excluded: Go test-runner functions (TestXxx / BenchmarkXxx / FuzzXxx / ExampleXxx in *_test.go) and kong/cobra-style *Cmd command types. For Go the graph now tracks TYPE usages too (issue #398) — a type used only as a field / parameter / composite-literal / embedding type counts as referenced, eliminating that whole false-positive class; the other tree-sitter languages still track call references only, so a type used only as a field type there can still look unreferenced. IMPORTANT — the rest are still CANDIDATES, not authoritative: name-based heuristic with known false positives — exported/public API used only by external callers, entry points (main), dynamic dispatch, reflection, and same-name collisions all surface here. Use as a starting point for manual review, never as a delete list. Output: candidates[] {path, language, kind, symbol} sorted by path + count + total_files.",
	...
	mcp.AddTool(s, &mcp.Tool{
		Name:        "coupling",
		Description: "Per-package afferent/efferent coupling + instability (Robert C. Martin's metrics) — the architecture-health report the import fan-out guard only gestures at. Resolves each Go file to its package import path (go.mod module path + directory) and counts distinct package→package import edges among first-party packages. For each package: afferent (Ca = how many first-party packages import it), efferent (Ce = how many it imports), instability (I = Ce/(Ca+Ce), 0=maximally stable/depended-upon, 1=maximally unstable/depends-on-others). Ranked most-depended-upon (high Ca) then most unstable (high I) — high Ca + high I marks a fragile hub: heavily relied upon yet itself volatile, the riskiest seam to change. Input: dir (the MODULE ROOT holding go.mod; default '.'), expr (defaults to is_source), top (cap; 0=all), plus the usual excludes / prune_build_artefacts / timeout_seconds. Output: module, packages[] {package, afferent, efferent, instability}, count. GO-ONLY — resolution keys on the go.mod module prefix; returns module=\"\" + empty packages when no go.mod is at dir. dirs[] uses the first root as the module root.",
	}, instrument(h.metrics, "coupling", h.couplingHandler))

	mcp.AddTool(s, &mcp.Tool{
		Name:        "unused_exports",
		Description: "List exported Go symbols (functions / types) that are referenced ONLY from within their own package — candidates to unexport and shrink the package's public API surface. The subtler complement to dead_code: dead_code finds symbols used nowhere; unused_exports finds symbols used somewhere, but never across a package boundary. Resolves each file to its package (go.mod module path + directory), then keeps exported symbols whose every referencing file lives in the defining package. Input: dir (the MODULE ROOT holding go.mod; default '.'), expr (defaults to is_source), plus the usual excludes / prune_build_artefacts / timeout_seconds. Output: module, candidates[] {symbol, kind: function|type, path, package} sorted by package then symbol, count. GO-ONLY (package resolution via the go.mod prefix; returns module=\"\" + empty when no go.mod at dir). HEURISTIC — reflection / framework dispatch (kong …Cmd, Go test entries) is excluded, but external consumers outside the walked tree, interface satisfaction, and same-name collisions across packages can still mislead; a review list, not an auto-unexport list. Uses the #398 Go type-usage references so type exports aren't wrongly flagged.",
	...
	mcp.AddTool(s, &mcp.Tool{
		Name:        "complexity",
		Description: "List functions ranked by cyclomatic complexity (worst-first) — find the gnarliest, hardest-to-maintain functions in a tree. Each row: {path, function, complexity, start_line, end_line, lines}. Complexity is gocyclo-style (1 + branch points: if / for / while / case / && / ||). Input: expr (defaults to is_source — narrow with language == \"go\"), top (cap, default 50), plus dir / dirs / excludes / respect_gitignore / timeout_seconds. Coverage: Go (stdlib AST) + the tree-sitter languages (Rust / TypeScript / JavaScript / Ruby / Swift / Kotlin / C / C++); other languages report nothing. Directionally accurate for RANKING hotspots — the exact number depends on per-grammar node coverage, so treat it as 'which functions are scariest', not a certified metric. For a file-level filter use the search tool's max_complexity attribute (e.g. 'is_source && max_complexity > 15'); this tool is the per-function drill-down.",
