...
| **Cross-file code graph + metrics** | `imported_by`, `find_definition`, `code_graph`, `who_calls`, `calls`, `impact`, `call_path`, `dead_code`, `test_gaps`, `coverage_gaps`, `complexity`, `coupling`, `unused_exports` | Reverse-dependency ("who imports X?"), symbol-definition lookup ("where is Y defined?"), project-wide overview (import hubs / high fan-out / duplicate defs), reverse call lookup ("who calls Y?"), forward call lookup ("what does Y call?"), transitive caller closure (`impact` — blast radius of changing Y), shortest call route between two functions (`call_path` — "how does A reach B?"), a candidate dead-code finder, an untested-code finder (`test_gaps` — prod functions no test references), precise per-function coverage gaps from a Go profile (`coverage_gaps`), per-function cyclomatic-complexity ranking, and per-package afferent/efferent coupling + instability (`coupling` — Martin metrics over first-party Go packages, the fragile-hub seams), and an over-export finder (`unused_exports` — exported Go symbols used only within their own package, the unexport-candidate complement to dead_code) — built from the per-file `imports` / `functions` / `type_names` / `references` / `max_complexity` data |
