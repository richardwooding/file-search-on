...
| **Cross-file code graph** | `imported_by`, `find_definition`, `code_graph`, `who_calls`, `calls`, `dead_code` | Reverse-dependency ("who imports X?"), symbol-definition lookup ("where is Y defined?"), project-wide overview (import hubs / high fan-out / duplicate defs), reverse call lookup ("who calls Y?"), forward call lookup ("what does Y call?"), and a candidate dead-code finder — built by inverting the per-file `imports` / `functions` / `type_names` / `references` lists |
