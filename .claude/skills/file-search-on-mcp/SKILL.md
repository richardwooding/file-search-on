...
| **Dedup & diff** | `find_duplicates`, `find_near_duplicates`, `find_duplicate_functions`, `diff_trees` | Byte-identical groups by sha256, fuzzy file groups via SimHash on body (auto-bumps threshold for source-heavy trees + strips boilerplate), **function-level** copy-paste clusters (SimHash per function span — the extract-this-helper finder), or cross-tree set ops (a-minus-b / intersect / mismatch) |
