...
Grouped into eight families. Full inputs / outputs / gotchas in [references/tools.md](references/tools.md).

| Family | Tools | Use for |
| --- | --- | --- |
| **Search & inspect** | `search`, `search_semantic`, `read_attributes`, `read_lines` | The everyday surface: walk a tree under a CEL filter, embed-rank by natural language, fetch attributes for one path, or pull a line range. `search_semantic` chunks source files **per function** and each hit reports `match_symbol` + `match_start_line`/`match_end_line` (the matching function), optionally inlining its code via `include_match_snippet` |
| **Aggregate** | `stats`, `churn_owners` | Histograms + totals bucketed by `group_by` (content_type, language, camera_make, ext, dir, …); or git ownership / bus-factor per directory (`churn_owners` — distinct authors, dominant maintainer + share, churn — ranked single-author-first to flag bus-factor risk) |
| **Dedup & diff** | `find_duplicates`, `find_near_duplicates`, `find_duplicate_functions`, `diff_trees`, `api_diff` | Byte-identical groups by sha256, fuzzy file groups via SimHash on body (auto-bumps threshold for source-heavy trees + strips boilerplate), **function-level** copy-paste clusters (SimHash per function span — the extract-this-helper finder), cross-tree set ops (a-minus-b / intersect / mismatch), or the exported-API breaking-change gate (`api_diff` — removed/added public functions & types between two trees) |
...
| **Cross-file code graph + metrics** | `imported_by`, `find_definition`, `references`, `code_graph`, `who_calls`, `calls`, `impact`, `call_path`, `trace`, `dead_code`, `test_gaps`, `coverage_gaps`, `complexity`, `coupling`, `circular`, `unused_exports`, `review` | Reverse-dependency ("who imports X?"), symbol-definition lookup ("where is Y defined?"), find-all-usages with file+line (`references` — the IDE find-references primitive: call / type / value sites), project-wide overview (import hubs / high fan-out / duplicate defs), reverse call lookup ("who calls Y?"), forward call lookup ("what does Y call?"), transitive caller closure (`impact` — blast radius of changing Y), shortest call route between two functions (`call_path` — "how does A reach B?"), both directions of a symbol's call graph in one shot (`trace` — callers + callees + optional impact closure, one graph build), a candidate dead-code finder, an untested-code finder (`test_gaps` — prod functions no test references), precise per-function coverage gaps from a Go profile (`coverage_gaps`), per-function cyclomatic + cognitive (SonarSource) complexity ranking, multi-language afferent/efferent coupling + instability (`coupling` — Martin metrics over first-party Go / Rust / JVM / C# / Python / JS-TS / PHP nodes, the fragile-hub seams), circular-dependency detection (`circular` — Tarjan SCCs in the same multi-language first-party import graph), an over-export finder (`unused_exports` — 9-language exported symbols used only within their own package, the unexport-candidate complement to dead_code), and a diff-scoped review gate (`review` — runs complexity / cognitive-complexity / dead-code over just the git-changed files and returns a pass/warn/fail verdict, the pre-commit / PR gate) — built from the per-file `imports` / `functions` / `type_names` / `references` / `max_complexity` data |
...
- Content Credentials (JPEG/PNG, unverified): `is_c2pa` — plus `c2pa_claim_generator` / `c2pa_ai_generated` / `c2pa_signed_by` / `c2pa_signed_at` (e.g. `is_image && c2pa_ai_generated`)
...
Fifteen baked recipes ship with the server. Discover via `list_presets`, run via `query_preset`. Each preset bakes a vetted CEL filter + sensible sort / limit defaults. The repo-aware ones auto-enable `with_git`.
