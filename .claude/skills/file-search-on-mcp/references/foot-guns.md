...
**Auto-bump (#274)**: when the candidate set is dominated by source files and the caller doesn't pass `threshold` explicitly, the server bumps the floor to `0.92`. This is the right default for source-heavy trees (it suppresses the cross-language SimHash convergence on license headers / mock generation / `// Code generated by …` banners). Pass `threshold` explicitly to opt out. To clamp huge clusters that DO survive, use `members_limit_per_group` (truncate each group's members) and `group_limit` (cap returned groups) — both added in #279.

Only text-shaped + structured-document types fingerprint (markdown / text / html / csv / json / xml / source/* / pdf / office / epub / email). Binary families return zero fingerprints and are silently excluded. Boilerplate strippers run before SimHash for source / markdown so the auto-bump isn't the only line of defence.
